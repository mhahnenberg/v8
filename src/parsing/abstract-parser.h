...
#ifndef V8_PARSING_ABSTRACT_PARSER_H_
#define V8_PARSING_ABSTRACT_PARSER_H_
...
//   template <typename Impl>
//   void ValidateDuplicate(AbstractParser<Impl>* parser) const;
//   template <typename Impl>
//   void ValidateStrictMode(AbstractParser<Impl>* parser) const;
// 
...
// template <typename Impl>
// class AbstractParser
//     : public ParserBase<Impl> {
//  public:
//   explicit AbstractParser(ParseInfo* info);
//   ~AbstractParser() {
...
//   friend class i::ParameterDeclarationParsingScope<ParserTypes<Impl>>;
//   friend class i::ArrowHeadParsingScope<ParserTypes<Impl>>;
...
//            impl()->scope()->AllowsLazyParsingWithoutUnresolvedVariables(
//                impl()->original_scope_);
//   }
// 
...
//    public:
//     ParsingModeScope(AbstractParser<Impl>* parser, Mode mode)
...
//     return impl()->scope()->NewTemporary(name);
//   }
// 
...
// 
//   // Called by ParseProgram after setting up the impl()->scanner.
...
//           &preparser_zone_, &scanner_, impl()->stack_limit_,
//           impl()->ast_value_factory(), impl()->pending_error_handler(),
//           impl()->runtime_call_stats_, impl()->logger_, impl()->flags(),
//           impl()->parsing_on_main_thread_);
//       reusable_preparser_->set_allow_eval_cache(impl()->allow_eval_cache());
...
//   Statement* BuildInitializationBlock(
//       typename ParserBase<Impl>::DeclarationParsingResult* parsing_result);
//   Expression* RewriteReturn(Expression* return_value, int pos);
//   Statement* RewriteSwitchStatement(SwitchStatement* switch_statement,
//                                     Scope* scope);
//   Block* RewriteCatchPattern(typename ParserBase<Impl>::CatchInfo* catch_info);
//   void ReportVarRedeclarationIn(const AstRawString* name, Scope* scope);
//   Statement* RewriteTryStatement(
//       Block* try_block, Block* catch_block, const SourceRange& catch_range,
//       Block* finally_block, const SourceRange& finally_range,
//       const typename ParserBase<Impl>::CatchInfo& catch_info, int pos);
...
//                             typename ParserBase<Impl>::ClassInfo* class_info,
//                             int class_token_pos);
//   void DeclareClassBrandVariable(
//       ClassScope* scope, typename ParserBase<Impl>::ClassInfo* class_info,
//       int class_token_pos);
//   void DeclarePrivateClassMember(
//       ClassScope* scope, const AstRawString* property_name,
//       ClassLiteralProperty* property, ClassLiteralProperty::Kind kind,
//       bool is_static, typename ParserBase<Impl>::ClassInfo* class_info);
//   void DeclarePublicClassMethod(
//       const AstRawString* class_name, ClassLiteralProperty* property,
//       bool is_constructor, typename ParserBase<Impl>::ClassInfo* class_info);
//   void DeclarePublicClassField(
//       ClassScope* scope, ClassLiteralProperty* property, bool is_static,
//       bool is_computed_name, typename ParserBase<Impl>::ClassInfo* class_info);
...
//                          typename ParserBase<Impl>::ClassInfo* class_info);
//   void AddClassStaticBlock(Block* block, typename ParserBase<Impl>::ClassInfo* class_info);
//   Expression* RewriteClassLiteral(
//       ClassScope* block_scope, const AstRawString* name,
//       typename ParserBase<Impl>::ClassInfo* class_info, int pos, int end_pos);
...
//     return impl()->scope()->GetDeclarationScope()->has_checked_syntax();
//   }
// 
//   void InitializeVariables(
//       ScopedPtrList<Statement>* statements, VariableKind kind,
//       const typename ParserBase<Impl>::DeclarationParsingResult::Declaration*
//           declaration);
// 
//   Block* RewriteForVarInLegacy(
//       const typename ParserBase<Impl>::ForInfo& for_info);
//   void DesugarBindingInForEachStatement(
//       typename ParserBase<Impl>::ForInfo* for_info, Block** body_block,
//       Expression** each_variable);
//   Block* CreateForEachStatementTDZ(
//       Block* init_block, const typename ParserBase<Impl>::ForInfo& for_info);
// 
//   Statement* DesugarLexicalBindingsInForStatement(
//       ForStatement* loop, Statement* init, Expression* cond, Statement* next,
//       Statement* body, Scope* inner_scope,
//       const typename ParserBase<Impl>::ForInfo& for_info);
// 
...
//     object_literal->CalculateEmitStore(impl()->main_zone());
...
//   // impl()->scanner- and preparser state to the initial one, before PreParsing
//   // the function. SkipFunction returns true if it correctly parsed the
//   // function, including cases where we detect an error. It returns false, if we
//   // needed to stop parsing or could not identify an error correctly, meaning
//   // the caller needs to fully reparse. In this case it resets the
//   // impl()->scanner and preparser state.
...
//     return identifier == impl()->ast_value_factory()->eval_string();
//   }
// 
//   V8_INLINE bool IsAsync(const AstRawString* identifier) const {
//     return identifier == impl()->ast_value_factory()->async_string();
//   }
// 
//   V8_INLINE bool IsArguments(const AstRawString* identifier) const {
//     return identifier == impl()->ast_value_factory()->arguments_string();
//   }
// 
...
//   }
// 
//   V8_INLINE VariableProxyExpression* AsIdentifierExpression(
//       Expression* expression) {
//     return expression->AsVariableProxyExpression();
//   }
// 
//   V8_INLINE bool IsConstructor(const AstRawString* identifier) const {
//     return identifier == impl()->ast_value_factory()->constructor_string();
//   }
// 
//   V8_INLINE bool IsName(const AstRawString* identifier) const {
//     return identifier == impl()->ast_value_factory()->name_string();
//   }
// 
...
//                impl()->ast_value_factory()->native_string();
//   }
// 
...
//     *default_string = impl()->ast_value_factory()->default_string();
//     *dot_default_string = impl()->ast_value_factory()->dot_default_string();
//   }
// 
//   // Functions for encapsulating the differences between parsing and preparsing;
//   // operations interleaved with the recursive descent.
//   V8_INLINE void PushLiteralName(const AstRawString* id) {
//     impl()->fni_.PushLiteralName(id);
//   }
// 
//   V8_INLINE void PushVariableName(const AstRawString* id) {
//     impl()->fni_.PushVariableName(id);
//   }
// 
//   V8_INLINE void PushPropertyName(Expression* expression) {
//     if (expression->IsPropertyName()) {
//       impl()->fni_.PushLiteralName(
//           expression->AsLiteral()->AsRawPropertyName());
//     } else {
//       impl()->fni_.PushLiteralName(
//           impl()->ast_value_factory()->computed_string());
//     }
//   }
// 
//   V8_INLINE void PushEnclosingName(const AstRawString* name) {
//     impl()->fni_.PushEnclosingName(name);
//   }
// 
//   V8_INLINE void AddFunctionForNameInference(FunctionLiteral* func_to_infer) {
//     impl()->fni_.AddFunction(func_to_infer);
//   }
// 
//   V8_INLINE void InferFunctionName() { impl()->fni_.Infer(); }
// 
...
//     args.Add(expr);
//     return impl()->factory()->NewCallRuntime(Runtime::kInlineToString, args,
//                                              expr->position());
//   }
// 
...
//                          impl()->ast_value_factory()->empty_string(), pos);
//   }
// 
...
//     return impl()->factory()->FailureExpression();
//   }
// 
//   template <typename T>
...
//     return impl()->ast_value_factory()->empty_string();
//   }
// 
//   // Producing data during the recursive descent.
//   V8_INLINE const AstRawString* GetSymbol() const {
//     const AstRawString* result =
//         impl()->scanner()->CurrentSymbol(impl()->ast_value_factory());
...
//     return impl()->scanner()->NextSymbol(impl()->ast_value_factory());
//   }
// 
//   V8_INLINE const AstRawString* GetNumberAsSymbol() const {
//     double double_value = impl()->scanner()->DoubleValue();
//     char array[100];
//     const char* string = DoubleToCString(double_value, ArrayVector(array));
//     return impl()->ast_value_factory()->GetOneByteString(string);
//   }
// 
//   class ThisExpression* ThisExpression() {
//     impl()->UseThis();
//     return impl()->factory()->ThisExpression();
//   }
// 
//   class ThisExpression* NewThisExpression(int pos) {
//     impl()->UseThis();
//     return impl()->factory()->NewThisExpression(pos);
//   }
// 
...
//       int start_position) {
//     VariableProxy* proxy =
//         impl()->factory()->ast_node_factory()->NewVariableProxy(
...
//       impl()->fni_.PushVariableName(name);
//     }
//     VariableProxy* proxy =
//         impl()->expression_scope()->NewVariable(name, start_position);
//     return impl()->factory()->NewVariableProxyExpression(proxy);
//   }
// 
//   V8_INLINE void DeclareIdentifier(const AstRawString* name,
//                                    int start_position) {
//     impl()->expression_scope()->Declare(name, start_position);
//   }
// 
...
//     return impl()->zone()->template New<ZonePtrList<Expression>>(size, impl()->zone());
//   }
//   V8_INLINE ZonePtrList<ObjectLiteral::Property>* NewObjectPropertyList(
//       int size) const {
//     return impl()->zone()->template New<ZonePtrList<ObjectLiteral::Property>>(size, impl()->zone());
//   }
//   V8_INLINE ZonePtrList<ClassLiteral::Property>* NewClassPropertyList(
//       int size) const {
//     return impl()->zone()->template New<ZonePtrList<ClassLiteral::Property>>(size, impl()->zone());
//   }
//   V8_INLINE ZonePtrList<ClassLiteral::StaticElement>* NewClassStaticElementList(
//       int size) const {
//     return impl()->zone()->template New<ZonePtrList<ClassLiteral::StaticElement>>(size, impl()->zone());
//   }
//   V8_INLINE ZonePtrList<Statement>* NewStatementList(int size) const {
//     return impl()->zone()->template New<ZonePtrList<Statement>>(size, impl()->zone());
//   }
// 
...
//         impl()->factory()->NewThrow(exception, pos), pos);
//   }
// 
...
//     auto parameter = parameters->scope->zone()->template New<ParserFormalParameters::Parameter>(
//       pattern, initializer,
//       impl()->scanner()->location().beg_pos,
...
//           is_simple ? parameter->name()
//                     : impl()->ast_value_factory()->empty_string(),
//           is_simple ? VariableMode::kVar : VariableMode::kTemporary,
//           is_optional, parameter->is_rest(),
//           impl()->ast_value_factory()->arguments_string(),
//           parameter->position);
//     }
//   }
// 
...
//         nary_op,
//         impl()->zone()->template New<NaryOperationSourceRanges>(impl()->zone(), range));
//   }
// 
...
//     if (source_range_map_ == nullptr) return;
//     source_range_map_->Insert(
//         node, impl()->zone()->template New<BlockSourceRanges>(continuation_position));
//   }
// 
...
//     if (source_range_map_ == nullptr) return;
//     source_range_map_->Insert(
//         node, impl()->zone()->template New<CaseClauseSourceRanges>(body_range));
//   }
// 
...
//     source_range_map_->Insert(
//         node->AsConditional(),
//         impl()->zone()->template New<ConditionalSourceRanges>(then_range, else_range));
//   }
// 
//   V8_INLINE void RecordFunctionLiteralSourceRange(FunctionLiteral* node) {
//     if (source_range_map_ == nullptr) return;
//     source_range_map_->Insert(node,
//                               impl()->zone()->template New<FunctionLiteralSourceRanges>());
//   }
// 
...
//                               impl()->zone()->template New<BinaryOperationSourceRanges>(right_range));
//   }
// 
...
//         impl()->zone()->template New<JumpStatementSourceRanges>(continuation_position));
//   }
// 
...
//     source_range_map_->Insert(
//         node->AsIfStatement(),
//         impl()->zone()->template New<IfStatementSourceRanges>(then_range, else_range));
//   }
// 
...
//     if (source_range_map_ == nullptr) return;
//     source_range_map_->Insert(
//         node, impl()->zone()->template New<IterationStatementSourceRanges>(body_range));
//   }
// 
...
//     if (source_range_map_ == nullptr) return;
//     source_range_map_->Insert(node,
//                               impl()->zone()->template New<ExpressionSourceRanges>(right_range));
//   }
// 
...
//                               impl()->zone()->template New<SuspendSourceRanges>(continuation_position));
//   }
// 
//   V8_INLINE void RecordSwitchStatementSourceRange(
//       Statement* node, int32_t continuation_position) {
//     if (source_range_map_ == nullptr) return;
//     source_range_map_->Insert(node->AsSwitchStatement(),
//                               impl()->zone()->template New<SwitchStatementSourceRanges>(
...
//     source_range_map_->Insert(throw_expr,
//                               impl()->zone()->template New<ThrowSourceRanges>(continuation_position));
//   }
// 
...
//     if (source_range_map_ == nullptr) return;
//     source_range_map_->Insert(
//         node, impl()->zone()->template New<TryCatchStatementSourceRanges>(body_range));
//   }
// 
...
//     if (source_range_map_ == nullptr) return;
//     source_range_map_->Insert(
//         node, impl()->zone()->template New<TryFinallyStatementSourceRanges>(body_range));
//   }
// 
...
// 
// template <typename Impl>
// FunctionLiteral* AbstractParser<Impl>::DefaultConstructor(
//     const AstRawString* name, bool call_super, int pos, int end_pos) {
...
// 
//   {
//     typename ParserBase<Impl>::FunctionState function_state(&impl()->function_state_, &impl()->scope_, function_scope);
// 
//     if (call_super) {
...
//       {
//         ScopedPtrList<Expression> args(impl()->pointer_buffer());
//         VariableProxy* proxy = impl()->factory()->NewVariableProxy(constructor_args);
//         Spread* spread_args = impl()->factory()->NewSpread(
//             impl()->factory()->NewVariableProxyExpression(proxy), pos, pos);
// 
...
// 
// template <typename Impl>
// void AbstractParser<Impl>::ReportUnexpectedTokenAt(Scanner::Location location,
//                                                    Token::Value token,
//                                                    MessageTemplate message) {
...
// template <typename Impl>
// bool AbstractParser<Impl>::ShortcutNumericLiteralBinaryExpression(
...
//         return true;
//       case Token::DIV:
//         *x = impl()->factory()->NewNumberLiteral(base::Divide(x_val, y_val),
//                                                  pos);
//         return true;
//       case Token::BIT_OR: {
...
//       }
//       case Token::EXP:
//         *x = impl()->factory()->NewNumberLiteral(
//             base::ieee754::pow(x_val, y_val), pos);
//         return true;
//       default:
//         break;
//     }
...
// 
// template <typename Impl>
// bool AbstractParser<Impl>::CollapseNaryExpression(Expression** x, Expression* y,
//                                                   Token::Value op, int pos,
//                                                   const SourceRange& range) {
...
// 
// template <typename Impl>
// Expression* AbstractParser<Impl>::BuildUnaryExpression(Expression* expression,
//                                                        Token::Value op,
//                                                        int pos) {
//   DCHECK_NOT_NULL(expression);
...
//       return impl()->factory()->NewBooleanLiteral(literal->ToBooleanIsFalse(),
//                                                   pos);
...
//         case Token::BIT_NOT:
//           return impl()->factory()->NewNumberLiteral(~DoubleToInt32(value),
//                                                      pos);
//         default:
//           break;
//       }
...
// 
// template <typename Impl>
// Expression* AbstractParser<Impl>::NewThrowError(Runtime::FunctionId id,
//                                                 MessageTemplate message,
...
//       impl()->factory()->NewCallRuntime(id, args, pos);
...
//     home_object_name = impl()->ast_value_factory()->dot_static_home_object_string();
//   } else {
//     home_object_name = impl()->ast_value_factory()->dot_home_object_string();
//   }
...
// 
// template <typename Impl>
// Expression* AbstractParser<Impl>::NewSuperCallReference(int pos) {
//   VariableProxy* new_target_proxy = impl()->NewUnresolved(
//       impl()->ast_value_factory()->new_target_string(), pos);
//   VariableProxyExpression* new_target_proxy_expr =
//       impl()->factory()->NewVariableProxyExpression(new_target_proxy);
//   VariableProxy* this_function_proxy = impl()->NewUnresolved(
//       impl()->ast_value_factory()->this_function_string(), pos);
//   VariableProxyExpression* this_function_proxy_expr =
//       impl()->factory()->NewVariableProxyExpression(this_function_proxy);
//   return impl()->factory()->NewSuperCallReference(
//       new_target_proxy_expr, this_function_proxy_expr, pos);
// }
// 
// template <typename Impl>
// Expression* AbstractParser<Impl>::NewTargetExpression(int pos) {
//   auto proxy = impl()->NewUnresolved(
//       impl()->ast_value_factory()->new_target_string(), pos);
...
// 
// template <typename Impl>
// Expression* AbstractParser<Impl>::ImportMetaExpression(int pos) {
//   ScopedPtrList<Expression> args(impl()->pointer_buffer());
//   return impl()->factory()->NewCallRuntime(Runtime::kInlineGetImportMetaObject,
//                                            args, pos);
// }
// 
// template <typename Impl>
// Expression* AbstractParser<Impl>::ExpressionFromLiteral(Token::Value token,
//                                                         int pos) {
//   switch (token) {
...
//           AstBigInt(impl()->scanner()->CurrentLiteralAsCString(impl()->zone())),
//           pos);
//     case Token::STRING: {
...
// 
// template <typename Impl>
// Expression* AbstractParser<Impl>::NewV8Intrinsic(
//     const AstRawString* name, const ScopedPtrList<Expression>& args, int pos) {
...
// template <typename Impl>
// Expression* AbstractParser<Impl>::NewV8RuntimeFunctionForFuzzing(
//     const Runtime::Function* function, const ScopedPtrList<Expression>& args,
//     int pos) {
//   CHECK(FLAG_fuzzing);
// 
...
// 
// template <typename Impl>
// AbstractParser<Impl>::AbstractParser(ParseInfo* info)
//     : ParserBase<Impl>(
...
//   bool can_compile_lazily =
//       impl()->flags().allow_lazy_compile() && !impl()->flags().is_eager();
// 
//   impl()->set_default_eager_compile_hint(
//       can_compile_lazily ? FunctionLiteral::kShouldLazyCompile
//                          : FunctionLiteral::kShouldEagerCompile);
//   allow_lazy_ = impl()->flags().allow_lazy_compile() &&
//                 impl()->flags().allow_lazy_parsing() &&
...
// 
// template <typename Impl>
// void AbstractParser<Impl>::InitializeEmptyScopeChain(ParseInfo* info) {
//   DCHECK_NULL(impl()->original_scope_);
//   DCHECK_NULL(info->script_scope());
//   DeclarationScope* script_scope = impl()->NewScriptScope(
//       impl()->flags().is_repl_mode() ? REPLMode::kYes : REPLMode::kNo);
//   info->set_script_scope(script_scope);
//   impl()->original_scope_ = script_scope;
// }
// 
// template <typename Impl>
// void AbstractParser<Impl>::DeserializeScopeChain(
...
//         IsArrowFunction(impl()->flags().function_kind())) {
...
// 
// template <typename Impl>
// void AbstractParser<Impl>::ParseProgram(
...
//     }
//     LOG(isolate, FunctionEvent(event_name, impl()->flags().script_id(), ms,
//                                start, end, "", 0));
//   }
// }
// 
// template <typename Impl>
// FunctionLiteral* AbstractParser<Impl>::DoParseProgram(Isolate* isolate,
//                                                       ParseInfo* info) {
...
//         &impl()->function_state_, &impl()->scope_, scope);
//     ScopedPtrList<Statement> body(impl()->pointer_buffer());
...
//       body.Add(impl()->factory()->NewExpressionStatement(initial_yield,
//                                                          kNoSourcePosition));
...
//         // into the main body.
//         typename ParserBase<Impl>::BlockT block = impl()->NullBlock();
//         {
//           typename ParserBase<Impl>::StatementListT statements(
//               impl()->pointer_buffer());
...
//       }
//       if (!impl()->has_error() &&
//           !impl()->module()->Validate(this->scope()->AsModuleScope(),
//                                       impl()->pending_error_handler(),
//                                       impl()->zone())) {
...
// 
// template <typename Impl>
// void AbstractParser<Impl>::PostProcessParseResult(Isolate* isolate,
//                                                   ParseInfo* info,
//                                                   FunctionLiteral* literal) {
...
// 
// template <typename Impl>
// ZonePtrList<const AstRawString>* AbstractParser<Impl>::PrepareWrappedArguments(
...
//   ZonePtrList<const AstRawString>* arguments_for_wrapped_function =
//       zone->template New<ZonePtrList<const AstRawString>>(arguments_length, zone);
//   for (int i = 0; i < arguments_length; i++) {
//     const AstRawString* argument_string =
//         impl()->ast_value_factory()->GetString(
...
// 
// template <typename Impl>
// void AbstractParser<Impl>::ParseWrapped(Isolate* isolate, ParseInfo* info,
//                                         ScopedPtrList<Statement>* body,
//                                         DeclarationScope* outer_scope,
//                                         Zone* zone) {
//   DCHECK(impl()->parsing_on_main_thread_);
...
//   typename ParserBase<Impl>::FunctionState function_state(
//       &impl()->function_state_, &impl()->scope_, outer_scope);
// 
...
// 
// template <typename Impl>
// void AbstractParser<Impl>::ParseREPLProgram(ParseInfo* info,
//                                             ScopedPtrList<Statement>* body,
//                                             DeclarationScope* scope) {
...
// 
// template <typename Impl>
// Expression* AbstractParser<Impl>::WrapREPLResult(Expression* value) {
...
//   return impl()->factory()->NewObjectLiteral(properties, false,
//                                              kNoSourcePosition, false);
// }
// 
// template <typename Impl>
// void AbstractParser<Impl>::ParseFunction(Isolate* isolate, ParseInfo* info,
//                                          Handle<SharedFunctionInfo> shared_info) {
//   // It's OK to use the Isolate & counters here, since this function is only
//   // called in the main thread.
//   DCHECK(impl()->parsing_on_main_thread_);
...
//     ClassScope::HeritageParsingScope heritage(
...
//           return nullptr;
//         }
//         if (!(impl()->peek_any_identifier() ||
//               impl()->peek() == Token::LPAREN)) {
//           CHECK(impl()->stack_overflow());
//           return nullptr;
//         }
//       }
...
//       {
//         typename ParserBase<Impl>::ParameterDeclarationParsingScope formals_scope(impl());
//         // Parsing patterns as variable reference expression creates
//         // impl()->NewUnresolved references in current scope. Enter arrow
//         // function scope for formal parameter parsing.
...
//         } else {
//           // BindingIdentifier
//           typename ParserBase<Impl>::ParameterParsingScope scope(impl(),
//                                                                  &formals);
...
// 
// template <typename Impl>
// Statement* AbstractParser<Impl>::ParseModuleItem() {
...
// 
// template <typename Impl>
// void AbstractParser<Impl>::ParseModuleItemList(ScopedPtrList<Statement>* body) {
//   // ecma262/#prod-Module
//   // Module :
//   //    ModuleBody?
//   //
...
// 
// template <typename Impl>
// const AstRawString* AbstractParser<Impl>::ParseModuleSpecifier() {
//   // ModuleSpecifier :
//   //    StringLiteral
// 
...
// 
// template <typename Impl>
// ZoneChunkList<typename AbstractParser<Impl>::ExportClauseData>*
// AbstractParser<Impl>::ParseExportClause(
//     Scanner::Location* reserved_loc,
//     Scanner::Location* string_literal_local_name_loc) {
//   // ExportClause :
//   //   '{' '}'
...
// 
// template <typename Impl>
// const AstRawString* AbstractParser<Impl>::ParseExportSpecifierName() {
...
// 
// template <typename Impl>
// ZonePtrList<const typename AbstractParser<Impl>::NamedImport>*
// AbstractParser<Impl>::ParseNamedImports(int pos) {
//   // NamedImports :
//   //   '{' '}'
//   //   '{' ImportsList '}'
//   //   '{' ImportsList ',' '}'
...
// 
//   auto result =
//       impl()->zone()->template New<ZonePtrList<const NamedImport>>(1, impl()->zone());
//   while (impl()->peek() != Token::RBRACE) {
//     const AstRawString* import_name = impl()->ParsePropertyName();
...
// 
// template <typename Impl>
// ImportAssertions* AbstractParser<Impl>::ParseImportAssertClause() {
//   // AssertClause :
//   //    assert '{' '}'
...
// 
// template <typename Impl>
// void AbstractParser<Impl>::ParseImportDeclaration() {
//   // ImportDeclaration :
...
// 
// template <typename Impl>
// Statement* AbstractParser<Impl>::ParseExportDefault() {
...
//       typename ParserBase<Impl>::AcceptINScope scope(this, true);
...
//       result = IgnoreCompletion(impl()->factory()->NewExpressionStatement(
//           assignment, kNoSourcePosition));
// 
//       impl()->ExpectSemicolon();
//       break;
//     }
...
//                                 impl()->ast_value_factory()->default_string(),
//                                 default_loc, impl()->zone());
//   }
// 
//   return result;
// }
// 
// template <typename Impl>
// const AstRawString* AbstractParser<Impl>::NextInternalNamespaceExportName() {
...
// 
// template <typename Impl>
// void AbstractParser<Impl>::ParseExportStar() {
//   int pos = impl()->position();
//   impl()->Consume(Token::MUL);
// 
...
// 
// template <typename Impl>
// Statement* AbstractParser<Impl>::ParseExportDeclaration() {
//   // ExportDeclaration:
//   //    'export' '*' 'from' ModuleSpecifier ';'
...
// 
// template <typename Impl>
// void AbstractParser<Impl>::DeclareUnboundVariable(const AstRawString* name,
//                                                   VariableMode mode,
//                                                   InitializationFlag init,
//                                                   int pos) {
//   bool was_added;
//   Variable* var =
//       DeclareVariable(name, NORMAL_VARIABLE, mode, init, impl()->scope(),
...
// 
// template <typename Impl>
// VariableProxy* AbstractParser<Impl>::DeclareBoundVariable(
//     const AstRawString* name, VariableMode mode, int pos) {
//   DCHECK_NOT_NULL(name);
//   VariableProxy* proxy = impl()->factory()->NewVariableProxy(
//       name, NORMAL_VARIABLE, impl()->position());
//   bool was_added;
//   Variable* var = DeclareVariable(
//       name, NORMAL_VARIABLE, mode, Variable::DefaultInitializationFlag(mode),
...
// 
// template <typename Impl>
// void AbstractParser<Impl>::DeclareAndBindVariable(VariableProxy* proxy,
//                                                   VariableKind kind,
//                                                   VariableMode mode,
...
// 
// template <typename Impl>
// Variable* AbstractParser<Impl>::DeclareVariable(const AstRawString* name,
//                                                 VariableKind kind,
//                                                 VariableMode mode,
//                                                 InitializationFlag init,
//                                                 Scope* scope, bool* was_added,
...
// 
// template <typename Impl>
// void AbstractParser<Impl>::Declare(Declaration* declaration,
//                                    const AstRawString* name,
...
// 
// template <typename Impl>
// Statement* AbstractParser<Impl>::BuildInitializationBlock(
//     typename ParserBase<Impl>::DeclarationParsingResult* parsing_result) {
...
// 
// template <typename Impl>
// Statement* AbstractParser<Impl>::DeclareFunction(
//     const AstRawString* variable_name, FunctionLiteral* function,
//     VariableMode mode, VariableKind kind, int beg_pos, int end_pos,
//     ZonePtrList<const AstRawString>* names) {
//   Declaration* declaration =
//       impl()->factory()->NewFunctionDeclaration(function, beg_pos);
//   bool was_added;
//   Declare(declaration, variable_name, kind, mode, kCreatedInitialized,
//           impl()->scope(), &was_added, beg_pos);
...
//     Token::Value init =
//         impl()->loop_nesting_depth() > 0 ? Token::ASSIGN : Token::INIT;
//     SloppyBlockFunctionStatement* statement =
//         impl()->factory()->NewSloppyBlockFunctionStatement(
//             end_pos, declaration->var(), init);
...
// 
// template <typename Impl>
// Statement* AbstractParser<Impl>::DeclareClass(
//     const AstRawString* variable_name, Expression* value,
//     ZonePtrList<const AstRawString>* names, int class_token_pos, int end_pos) {
//   VariableProxy* proxy =
...
// 
//   Assignment* assignment = impl()->factory()->NewAssignment(
//       Token::INIT, proxy_expr, value, class_token_pos);
//   return IgnoreCompletion(
//       impl()->factory()->NewExpressionStatement(assignment, kNoSourcePosition));
// }
// 
// template <typename Impl>
// Statement* AbstractParser<Impl>::DeclareNative(const AstRawString* name,
//                                                int pos) {
...
//   VariableProxyExpression* proxy_expr =
//       impl()->factory()->NewVariableProxyExpression(proxy);
//   NativeFunctionLiteral* lit = impl()->factory()->NewNativeFunctionLiteral(
//       name, impl()->extension_, kNoSourcePosition);
//   return impl()->factory()->NewExpressionStatement(
//       impl()->factory()->NewAssignment(Token::INIT, proxy_expr, lit,
//                                        kNoSourcePosition),
//       pos);
// }
// 
// template <typename Impl>
// Block* AbstractParser<Impl>::IgnoreCompletion(Statement* statement) {
...
// 
// template <typename Impl>
// Expression* AbstractParser<Impl>::RewriteReturn(Expression* return_value,
//                                                 int pos) {
...
//     VariableProxy* proxy = impl()->factory()->NewVariableProxy(temp);
//     Assignment* assign = impl()->factory()->NewAssignment(
//         Token::ASSIGN, impl()->factory()->NewVariableProxyExpression(proxy),
//         return_value, pos);
// 
//     // temp === undefined
...
//     // ParseFunctionBody.
//     return_value = impl()->factory()->NewConditional(
//         is_undefined, impl()->factory()->ThisExpression(),
//         impl()->factory()->NewVariableProxyExpression(proxy), pos);
//   }
//   return return_value;
// }
// 
// template <typename Impl>
// Statement* AbstractParser<Impl>::RewriteSwitchStatement(
//     SwitchStatement* switch_statement, Scope* scope) {
...
//   VariableProxy* tag_variable_proxy =
//       impl()->factory()->NewVariableProxy(tag_variable);
//   Assignment* tag_assign = impl()->factory()->NewAssignment(
//       Token::ASSIGN,
//       impl()->factory()->NewVariableProxyExpression(tag_variable_proxy), tag,
//       tag->position());
...
// 
//   switch_statement->set_tag(
//       impl()->factory()->NewVariableProxyExpression(tag_variable_proxy));
...
// 
// template <typename Impl>
// void AbstractParser<Impl>::InitializeVariables(
//     ScopedPtrList<Statement>* statements, VariableKind kind,
//     const typename ParserBase<Impl>::DeclarationParsingResult::Declaration*
//         declaration) {
//   if (impl()->has_error()) return;
...
// 
// template <typename Impl>
// Block* AbstractParser<Impl>::RewriteCatchPattern(
//     typename ParserBase<Impl>::CatchInfo* catch_info) {
//   DCHECK_NOT_NULL(catch_info->pattern);
// 
//   VariableProxy* catch_info_variable_proxy =
//       impl()->factory()->NewVariableProxy(catch_info->variable);
//   typename ParserBase<Impl>::DeclarationParsingResult::Declaration decl(
//       catch_info->pattern,
//       impl()->factory()->NewVariableProxyExpression(catch_info_variable_proxy));
// 
...
// 
// template <typename Impl>
// void AbstractParser<Impl>::ReportVarRedeclarationIn(const AstRawString* name,
//                                                     Scope* scope) {
...
// 
// template <typename Impl>
// Statement* AbstractParser<Impl>::RewriteTryStatement(
//     Block* try_block, Block* catch_block, const SourceRange& catch_range,
//     Block* finally_block, const SourceRange& finally_range,
//     const typename ParserBase<Impl>::CatchInfo& catch_info, int pos) {
...
//     statement = impl()->factory()->NewTryCatchStatement(
//         try_block, catch_info.scope, catch_block, kNoSourcePosition);
...
//     TryFinallyStatement* stmt = impl()->factory()->NewTryFinallyStatement(
//         try_block, finally_block, pos);
...
// 
// template <typename Impl>
// void AbstractParser<Impl>::ParseAndRewriteGeneratorFunctionBody(
//     int pos, FunctionKind kind, ScopedPtrList<Statement>* body) {
...
// 
// template <typename Impl>
// void AbstractParser<Impl>::ParseAndRewriteAsyncGeneratorFunctionBody(
//     int pos, FunctionKind kind, ScopedPtrList<Statement>* body) {
...
//     ScopedPtrList<Statement> statements(impl()->pointer_buffer());
//     Expression* initial_yield = BuildInitialYield(pos, kind);
//     statements.Add(impl()->factory()->NewExpressionStatement(
//         initial_yield, kNoSourcePosition));
...
//     reject_args.Add(impl()->factory()->NewVariableProxyExpression(
//         impl()->factory()->NewVariableProxy(
//             impl()->function_state_->scope()->generator_object_var())));
//     reject_args.Add(impl()->factory()->NewVariableProxyExpression(
//         impl()->factory()->NewVariableProxy(catch_scope->catch_variable())));
// 
...
//     close_call = impl()->factory()->NewCallRuntime(
...
// 
// template <typename Impl>
// void AbstractParser<Impl>::DeclareFunctionNameVar(
...
// template <typename Impl>
// Block* AbstractParser<Impl>::RewriteForVarInLegacy(
//     const typename ParserBase<Impl>::ForInfo& for_info) {
//   auto& decl =
//       for_info.parsing_result.declarations[0];
//   if (!IsLexicalVariableMode(for_info.parsing_result.descriptor.mode) &&
//       decl.initializer != nullptr &&
//       decl.pattern->IsVariableProxyExpression()) {
//     ++use_counts_[v8::Isolate::kForInInitializer];
//     const AstRawString* name =
//         decl.pattern->AsVariableProxyExpression()->raw_name();
//     VariableProxy* single_var = impl()->NewUnresolved(name);
//     Block* init_block = impl()->factory()->NewBlock(2, true);
//     init_block->statements()->Add(
//         impl()->factory()->NewExpressionStatement(
//             impl()->factory()->NewAssignment(
//                 Token::ASSIGN,
//                 impl()->factory()->NewVariableProxyExpression(single_var),
...
// //   }
// template <typename Impl>
// void AbstractParser<Impl>::DesugarBindingInForEachStatement(
//     typename ParserBase<Impl>::ForInfo* for_info, Block** body_block,
//     Expression** each_variable) {
...
//   ScopedPtrList<Statement> each_initialization_statements(
//       impl()->pointer_buffer());
//   DCHECK_IMPLIES(!impl()->has_error(), decl.pattern != nullptr);
//   decl.initializer = impl()->factory()->NewVariableProxyExpression(
//       impl()->factory()->NewVariableProxy(temp, for_info->position));
//   InitializeVariables(&each_initialization_statements, NORMAL_VARIABLE,
//                       &decl);
// 
//   *body_block = impl()->factory()->NewBlock(3, false);
//   (*body_block)
//       ->statements()
//       ->Add(impl()->factory()->NewBlock(true, each_initialization_statements),
//             impl()->zone());
//   *each_variable = impl()->factory()->NewVariableProxyExpression(
//       impl()->factory()->NewVariableProxy(temp, for_info->position));
// }
// 
// // Create a TDZ for any lexically-bound names in for in/of statements.
// template <typename Impl>
// Block* AbstractParser<Impl>::CreateForEachStatementTDZ(
//     Block* init_block, const typename ParserBase<Impl>::ForInfo& for_info) {
//   if (IsLexicalVariableMode(for_info.parsing_result.descriptor.mode)) {
...
// 
// template <typename Impl>
// Statement* AbstractParser<Impl>::DesugarLexicalBindingsInForStatement(
//     ForStatement* loop, Statement* init, Expression* cond, Statement* next,
//     Statement* body, Scope* inner_scope,
//     const typename ParserBase<Impl>::ForInfo& for_info) {
...
//   for (const AstRawString* bound_name : for_info.bound_names) {
//     VariableProxy* proxy = impl()->NewUnresolved(bound_name);
//     VariableProxyExpression* proxy_expr =
//         impl()->factory()->NewVariableProxyExpression(proxy);
//     Variable* temp = NewTemporary(temp_name);
//     VariableProxy* temp_proxy = impl()->factory()->NewVariableProxy(temp);
//     VariableProxyExpression* temp_proxy_expr =
//         impl()->factory()->NewVariableProxyExpression(temp_proxy);
//     Assignment* assignment = impl()->factory()->NewAssignment(
//         Token::ASSIGN, temp_proxy_expr, proxy_expr, kNoSourcePosition);
//     Statement* assignment_statement = impl()->factory()->NewExpressionStatement(
//         assignment, kNoSourcePosition);
...
//     VariableProxy* first_proxy = impl()->factory()->NewVariableProxy(first);
//     VariableProxyExpression* first_proxy_expr =
//         impl()->factory()->NewVariableProxyExpression(first_proxy);
...
//       impl()->factory()->NewForStatement(kNoSourcePosition);
...
//           impl()->factory()->NewVariableProxy(temps.at(i));
//       VariableProxyExpression* temp_proxy_expr =
//           impl()->factory()->NewVariableProxyExpression(temp_proxy);
//       Assignment* assignment = impl()->factory()->NewAssignment(
//           Token::INIT, proxy_expr, temp_proxy_expr, kNoSourcePosition);
//       Statement* assignment_statement =
//           impl()->factory()->NewExpressionStatement(assignment,
//                                                     kNoSourcePosition);
...
//       {
//         Expression* const1 =
//             impl()->factory()->NewSmiLiteral(1, kNoSourcePosition);
//         VariableProxy* first_proxy = impl()->factory()->NewVariableProxy(first);
//         VariableProxyExpression* first_proxy_expr =
//             impl()->factory()->NewVariableProxyExpression(first_proxy);
//         compare = impl()->factory()->NewCompareOperation(
//             Token::EQ, first_proxy_expr, const1, kNoSourcePosition);
//       }
...
//             impl()->factory()->NewVariableProxyExpression(first_proxy);
//         Expression* const0 =
//             impl()->factory()->NewSmiLiteral(0, kNoSourcePosition);
//         Assignment* assignment = impl()->factory()->NewAssignment(
//             Token::ASSIGN, first_proxy_expr, const0, kNoSourcePosition);
//         clear_first = impl()->factory()->NewExpressionStatement(
//             assignment, kNoSourcePosition);
//       }
...
//     {
//       VariableProxy* flag_proxy = impl()->factory()->NewVariableProxy(flag);
//       VariableProxyExpression* flag_proxy_expr =
//           impl()->factory()->NewVariableProxyExpression(flag_proxy);
//       Expression* const1 =
//           impl()->factory()->NewSmiLiteral(1, kNoSourcePosition);
//       Assignment* assignment = impl()->factory()->NewAssignment(
...
//       VariableProxyExpression* flag_proxy_expr =
//           impl()->factory()->NewVariableProxyExpression(flag_proxy);
//       flag_cond = impl()->factory()->NewCompareOperation(
//           Token::EQ, flag_proxy_expr, const1, kNoSourcePosition);
//     }
// 
...
//         Expression* const0 =
//             impl()->factory()->NewSmiLiteral(0, kNoSourcePosition);
//         compound_next = impl()->factory()->NewAssignment(
//             Token::ASSIGN, flag_proxy_expr, const0, kNoSourcePosition);
//       }
// 
...
//             impl()->factory()->NewVariableProxyExpression(temp_proxy);
//         VariableProxy* proxy = impl()->factory()->NewVariableProxy(
//             inner_vars.at(i), inner_var_proxy_pos);
//         VariableProxyExpression* proxy_expr =
//             impl()->factory()->NewVariableProxyExpression(proxy);
//         Assignment* assignment = impl()->factory()->NewAssignment(
//             Token::ASSIGN, temp_proxy_expr, proxy_expr, kNoSourcePosition);
...
//       }
// 
//       compound_next_statement = impl()->factory()->NewExpressionStatement(
//           compound_next, kNoSourcePosition);
//     }
// 
...
//       Statement* if_flag_break = impl()->factory()->NewIfStatement(
//           compare, stop, empty, kNoSourcePosition);
...
// 
// template <typename Impl>
// void ParserFormalParameters::ValidateDuplicate(
//     AbstractParser<Impl>* parser) const {
//   if (has_duplicate()) {
//     parser->ReportMessageAt(duplicate_loc, MessageTemplate::kParamDupe);
//   }
// }
// template <typename Impl>
// void ParserFormalParameters::ValidateStrictMode(
//     AbstractParser<Impl>* parser) const {
...
// 
// template <typename Impl>
// void AbstractParser<Impl>::AddArrowFunctionFormalParameters(
...
// 
// template <typename Impl>
// void AbstractParser<Impl>::DeclareArrowFunctionFormalParameters(
...
// 
// template <typename Impl>
// void AbstractParser<Impl>::PrepareGeneratorVariables() {
...
// 
// template <typename Impl>
// FunctionLiteral* AbstractParser<Impl>::ParseFunctionLiteral(
...
//   // impl()->scanner()->stream()->can_be_cloned()); printf("Stream can be cloned
//   // for parallel access: %d\n",
//   // impl()->scanner()->stream()->can_be_cloned_for_parallel_access());
...
//   impl()->CheckFunctionName(language_mode, function_name,
//                             function_name_validity, function_name_location);
// 
//   if (is_strict(language_mode)) {
//     impl()->CheckStrictOctalLiteral(scope->start_position(),
//                                     scope->end_position());
//   }
// 
...
//     info()->parallel_tasks()->EnqueueCompileTask(info(), function_name,
//                                                  function_literal);
//   }
// 
...
// 
// template <typename Impl>
// bool AbstractParser<Impl>::SkipFunction(
...
//   typename ParserBase<Impl>::FunctionState function_state(
//       &impl()->function_state_, &impl()->scope_, function_scope);
...
//             impl()->main_zone(), function_scope->start_position(),
//             &end_position, num_parameters, function_length,
//             &num_inner_functions, &uses_super_property, &language_mode);
// 
...
//   } else {
//     DCHECK(!impl()->pending_error_handler()->stack_overflow());
//     impl()->set_allow_eval_cache(reusable_preparser()->impl()->allow_eval_cache());
// 
...
//     }
//     function_scope->AnalyzePartially(impl(), impl()->factory(),
//                                      impl()->MaybeParsingArrowhead());
//   }
// 
//   return true;
// }
// 
// template <typename Impl>
// Block* AbstractParser<Impl>::BuildParameterInitializationBlock(
//     const ParserFormalParameters& parameters) {
...
//   int index = 0;
//   for (auto parameter : parameters.params) {
//     Expression* initial_value = impl()->factory()->NewVariableProxyExpression(
//         impl()->factory()->NewVariableProxy(
//             parameters.scope->parameter(index)));
//     if (parameter->initializer() != nullptr) {
//       // IS_UNDEFINED($param) ? initializer : $param
// 
//       auto condition = impl()->factory()->NewCompareOperation(
//           Token::EQ_STRICT,
//           impl()->factory()->NewVariableProxyExpression(
//               impl()->factory()->NewVariableProxy(
//                   parameters.scope->parameter(index))),
//           impl()->factory()->NewUndefinedLiteral(kNoSourcePosition),
//           kNoSourcePosition);
...
//     }
// 
//     typename ParserBase<Impl>::BlockState block_state(
//         &impl()->scope_, impl()->scope()->AsDeclarationScope());
//     typename ParserBase<Impl>::DeclarationParsingResult::Declaration decl(parameter->pattern,
//                                                 initial_value);
...
// 
// template <typename Impl>
// Scope* AbstractParser<Impl>::NewHiddenCatchScope() {
...
// 
// template <typename Impl>
// Block* AbstractParser<Impl>::BuildRejectPromiseOnException(Block* inner_block,
//                                                            REPLMode repl_mode) {
//   // try {
//   //   <inner_block>
//   // } catch (.catch) {
...
//   {
//     ScopedPtrList<Expression> args(impl()->pointer_buffer());
//     args.Add(impl()->factory()->NewVariableProxyExpression(
//         impl()->factory()->NewVariableProxy(
//             impl()->function_state_->scope()->generator_object_var())));
//     args.Add(impl()->factory()->NewVariableProxyExpression(
//         impl()->factory()->NewVariableProxy(catch_scope->catch_variable())));
//     args.Add(impl()->factory()->NewBooleanLiteral(
//         impl()->function_state_->CanSuspend(), kNoSourcePosition));
...
// 
// template <typename Impl>
// Expression* AbstractParser<Impl>::BuildInitialYield(int pos,
//                                                     FunctionKind kind) {
//   Expression* yield_result = impl()->factory()->NewVariableProxyExpression(
//       impl()->factory()->NewVariableProxy(
//           impl()->function_state_->scope()->generator_object_var()));
...
//   return impl()->factory()->NewYield(yield_result,
//                                      impl()->scope()->start_position(),
//                                      Suspend::kOnExceptionThrow);
// }
// 
// template <typename Impl>
// void AbstractParser<Impl>::ParseFunction(
...
//     int* suspend_count,
//     ZonePtrList<const AstRawString>* arguments_for_wrapped_function) {
//   typename ParserBase<Impl>::FunctionParsingScope function_parsing_scope(impl());
//   ParsingModeScope mode(this, allow_lazy_ ? PARSE_LAZILY : PARSE_EAGERLY);
// 
//   typename ParserBase<Impl>::FunctionState function_state(
//       &impl()->function_state_, &impl()->scope_, function_scope);
...
// 
// template <typename Impl>
// void AbstractParser<Impl>::DeclareClassVariable(
//     ClassScope* scope, const AstRawString* name,
//     typename ParserBase<Impl>::ClassInfo* class_info, int class_token_pos) {
// #ifdef DEBUG
...
//   Variable* class_variable = scope->DeclareClassVariable(
//       impl()->ast_value_factory(), name, class_token_pos);
//   Declaration* declaration =
//       impl()->factory()->NewVariableDeclaration(class_token_pos);
...
// template <typename Impl>
// Variable* AbstractParser<Impl>::CreateSyntheticContextVariable(
//     const AstRawString* name) {
//   VariableProxy* proxy =
...
// 
// template <typename Impl>
// Variable* AbstractParser<Impl>::CreatePrivateNameVariable(
//     ClassScope* scope, VariableMode mode, IsStaticFlag is_static_flag,
//     const AstRawString* name) {
//   DCHECK_NOT_NULL(name);
...
// 
// template <typename Impl>
// void AbstractParser<Impl>::DeclarePublicClassField(
//     ClassScope* scope, ClassLiteralProperty* property, bool is_static,
//     bool is_computed_name, typename ParserBase<Impl>::ClassInfo* class_info) {
//   if (is_static) {
...
// 
// template <typename Impl>
// void AbstractParser<Impl>::DeclarePrivateClassMember(ClassScope* scope,
...
// //   - properties
// template <typename Impl>
// void AbstractParser<Impl>::DeclarePublicClassMethod(
//     const AstRawString* class_name, ClassLiteralProperty* property,
//     bool is_constructor, typename ParserBase<Impl>::ClassInfo* class_info) {
//   if (is_constructor) {
...
//         class_name != nullptr
//             ? impl()->ast_value_factory()->NewConsString(class_name)
//             : nullptr);
//     return;
//   }
// 
...
// 
// template <typename Impl>
// void AbstractParser<Impl>::AddClassStaticBlock(Block* block, typename ParserBase<Impl>::ClassInfo* class_info) {
...
// 
// template <typename Impl>
// FunctionLiteral* AbstractParser<Impl>::CreateInitializerFunction(
//     const char* name, DeclarationScope* scope, Statement* initializer_stmt) {
...
// template <typename Impl>
// Expression* AbstractParser<Impl>::RewriteClassLiteral(
//     ClassScope* block_scope, const AstRawString* name,
//     typename ParserBase<Impl>::ClassInfo* class_info, int pos, int end_pos) {
...
// 
// template <typename Impl>
// void AbstractParser<Impl>::InsertShadowingVarBindingInitializers(
//     Block* inner_block) {
...
//     VariableProxy* to = impl()->NewUnresolved(name);
//     VariableProxyExpression* to_expr =
//         impl()->factory()->NewVariableProxyExpression(to);
//     VariableProxy* from = impl()->factory()->NewVariableProxy(parameter);
//     VariableProxyExpression* from_expr =
//         impl()->factory()->NewVariableProxyExpression(from);
//     Expression* assignment = impl()->factory()->NewAssignment(
//         Token::ASSIGN, to_expr, from_expr, kNoSourcePosition);
//     Statement* statement = impl()->factory()->NewExpressionStatement(
//         assignment, kNoSourcePosition);
...
// 
// template <typename Impl>
// void AbstractParser<Impl>::InsertSloppyBlockFunctionVarBindings(
//     DeclarationScope* scope) {
...
//       scope->outer_scope() == impl()->original_scope_) {
//     return;
//   }
...
// template <typename Impl>
// template <typename LocalIsolate>
// void AbstractParser<Impl>::HandleSourceURLComments(LocalIsolate* isolate,
...
// 
// template <typename Impl>
// void AbstractParser<Impl>::UpdateStatistics(Isolate* isolate,
//                                             Handle<Script> script) {
//   // Move statistics to Isolate.
//   for (int feature = 0; feature < v8::Isolate::kUseCounterFeatureCount;
//        ++feature) {
...
// 
// template <typename Impl>
// void AbstractParser<Impl>::ParseOnBackground(ParseInfo* info,
//                                              int start_position,
//                                              int end_position,
//                                              int function_literal_id) {
//   RuntimeCallTimerScope runtimeTimer(
//       impl()->runtime_call_stats_,
//       RuntimeCallCounterId::kParseBackgroundProgram);
...
// 
// template <typename Impl>
// typename AbstractParser<Impl>::TemplateLiteralState
// AbstractParser<Impl>::OpenTemplateLiteral(int pos) {
//   return impl()->zone()->template New<TemplateLiteral>(impl()->zone(), pos);
// }
// 
// template <typename Impl>
// void AbstractParser<Impl>::AddTemplateSpan(TemplateLiteralState* state,
//                                            bool should_cook, bool tail) {
//   int end = impl()->scanner()->location().end_pos - (tail ? 1 : 2);
//   const AstRawString* raw =
//       impl()->scanner()->CurrentRawSymbol(impl()->ast_value_factory());
//   if (should_cook) {
//     const AstRawString* cooked =
//         impl()->scanner()->CurrentSymbol(impl()->ast_value_factory());
...
// 
// template <typename Impl>
// void AbstractParser<Impl>::AddTemplateExpression(TemplateLiteralState* state,
//                                                  Expression* expression) {
//   (*state)->AddExpression(expression, impl()->zone());
// }
// 
// template <typename Impl>
// Expression* AbstractParser<Impl>::CloseTemplateLiteral(
...
//     }
//     return impl()->factory()->NewTemplateLiteral(cooked_strings, expressions,
//                                                  pos);
//   } else {
//     // GetTemplateObject
//     Expression* template_object = impl()->factory()->NewGetTemplateObject(
//         cooked_strings, raw_strings, pos);
// 
//     // Call TagFn
...
// 
// template <typename Impl>
// ArrayLiteral* AbstractParser<Impl>::ArrayLiteralFromListWithSpread(
//     const ScopedPtrList<Expression>& list) {
...
//   return impl()->factory()->NewArrayLiteral(list, first_spread,
//                                             kNoSourcePosition);
// }
// 
// template <typename Impl>
// Expression* AbstractParser<Impl>::SpreadCall(
//     Expression* function, const ScopedPtrList<Expression>& args_list, int pos,
...
//     return impl()->factory()->NewCall(function, args_list, pos, Call::NOT_EVAL,
//                                       optional_chain);
//   }
// 
//   ScopedPtrList<Expression> args(impl()->pointer_buffer());
...
//     } else {
//       Variable* temp =
//           NewTemporary(impl()->ast_value_factory()->empty_string());
//       VariableProxy* obj = impl()->factory()->NewVariableProxy(temp);
//       VariableProxyExpression* obj_expr =
//           impl()->factory()->NewVariableProxyExpression(obj);
//       Assignment* assign_obj = impl()->factory()->NewAssignment(
//           Token::ASSIGN, obj_expr, function->AsProperty()->obj(),
//           kNoSourcePosition);
//       function = impl()->factory()->NewProperty(
//           assign_obj, function->AsProperty()->key(), kNoSourcePosition,
//           optional_chain);
//       args.Add(function);
//       obj = impl()->factory()->NewVariableProxy(temp);
//       obj_expr = impl()->factory()->NewVariableProxyExpression(obj);
//       args.Add(obj_expr);
//     }
//   } else {
//     // Non-method calls
//     args.Add(function);
//     args.Add(impl()->factory()->NewUndefinedLiteral(kNoSourcePosition));
//   }
//   args.Add(ArrayLiteralFromListWithSpread(args_list));
//   return impl()->factory()->NewCallRuntime(Context::REFLECT_APPLY_INDEX, args,
//                                            pos);
// }
// 
// template <typename Impl>
// Expression* AbstractParser<Impl>::SpreadCallNew(
//     Expression* function, const ScopedPtrList<Expression>& args_list, int pos) {
//   if (OnlyLastArgIsSpread(args_list)) {
//     // Handle in BytecodeGenerator.
//     return impl()->factory()->NewCallNew(function, args_list, pos);
//   }
//   ScopedPtrList<Expression> args(impl()->pointer_buffer());
//   args.Add(function);
//   args.Add(ArrayLiteralFromListWithSpread(args_list));
// 
//   return impl()->factory()->NewCallRuntime(Context::REFLECT_CONSTRUCT_INDEX,
//                                            args, pos);
// }
// 
// template <typename Impl>
// void AbstractParser<Impl>::SetLanguageMode(Scope* scope, LanguageMode mode) {
...
// 
// template <typename Impl>
// void AbstractParser<Impl>::SetAsmModule() {
...
// 
// template <typename Impl>
// Expression* AbstractParser<Impl>::ExpressionListToExpression(
...
//                                                  args.at(1)->position());
//   }
//   NaryOperation* result = impl()->factory()->NewNaryOperation(
//       Token::COMMA, expr, args.length() - 1);
...
// template <typename Impl>
// void AbstractParser<Impl>::RewriteAsyncFunctionBody(
//     ScopedPtrList<Statement>* body, Block* block, Expression* return_value,
//     REPLMode repl_mode) {
...
// 
// template <typename Impl>
// void AbstractParser<Impl>::SetFunctionNameFromPropertyName(
//     LiteralProperty* property, const AstRawString* name,
...
// 
// template <typename Impl>
// void AbstractParser<Impl>::SetFunctionNameFromIdentifierRef(
...
// 
// template <typename Impl>
// void AbstractParser<Impl>::SetFunctionName(Expression* value,
//                                            const AstRawString* name,
//                                            const AstRawString* prefix) {
...
// 
// template <typename Impl>
// Statement* AbstractParser<Impl>::CheckCallable(Variable* var, Expression* error,
//                                                int pos) {
...
//         Token::TYPEOF,
//         impl()->factory()->NewVariableProxyExpression(
//             impl()->factory()->NewVariableProxy(var)),
//         nopos);
...
//         impl()->factory()->NewExpressionStatement(error, pos);
// 
...
template <typename Impl>
class AbstractParser : public NON_EXPORTED_BASE(ParserBase<Impl>) {
 public:
  explicit AbstractParser(ParseInfo* info);
  ~AbstractParser() {
    delete reusable_preparser_;
 ...
 private:
  ...
  using AcceptINScope = typename ParserBase<Impl>::AcceptINScope;
  using BlockState = typename ParserBase<Impl>::BlockState;
  using BlockT = typename ParserBase<Impl>::BlockT;
  using CatchInfo = typename ParserBase<Impl>::CatchInfo;
  using ClassInfo = typename ParserBase<Impl>::ClassInfo;
  using DeclarationParsingResult = typename ParserBase<Impl>::DeclarationParsingResult;
  using ForInfo = typename ParserBase<Impl>::ForInfo;
  using FunctionParsingScope = typename ParserBase<Impl>::FunctionParsingScope;
  using FunctionState = typename ParserBase<Impl>::FunctionState;
  using ParameterDeclarationParsingScope = typename ParserBase<Impl>::ParameterDeclarationParsingScope;
  using ParameterParsingScope = typename ParserBase<Impl>::ParameterParsingScope;
  using StatementListT = typename ParserBase<Impl>::StatementListT;
  ...
  PreParser* reusable_preparser() {
    if (reusable_preparser_ == nullptr) {
      reusable_preparser_ = new PreParser(
          &impl()->preparser_zone_, &scanner_, impl()->stack_limit_, impl()->ast_value_factory(),
          impl()->pending_error_handler(), impl()->runtime_call_stats_, impl()->logger_, impl()->flags(),
          impl()->parsing_on_main_thread_);
      reusable_preparser_->set_allow_eval_cache(impl()->allow_eval_cache());
      preparse_data_buffer_.reserve(128);
    }
  ...
  V8_INLINE void PushPropertyName(Expression* expression) {
    if (expression->IsPropertyName()) {
      impl()->fni_.PushLiteralName(expression->AsLiteral()->AsRawPropertyName());
    } else {
      impl()->fni_.PushLiteralName(impl()->ast_value_factory()->computed_string());
    }
  }
  ...
  V8_INLINE static std::nullptr_t NullStatement() { return nullptr; }
  V8_INLINE static std::nullptr_t NullBlock() { return nullptr; }
  Expression* FailureExpression() { return impl()->factory()->FailureExpression(); }
  ...
  // Producing data during the recursive descent.
  V8_INLINE const AstRawString* GetSymbol() const {
    const AstRawString* result = impl()->scanner()->CurrentSymbol(impl()->ast_value_factory());
    DCHECK_NOT_NULL(result);
    return result;
  }
  ...
  V8_INLINE VariableProxyExpression* ExpressionFromPrivateName(
      PrivateNameScopeIterator* private_name_scope, const AstRawString* name,
      int start_position) {
    VariableProxy* proxy = impl()->factory()->ast_node_factory()->NewVariableProxy(
        name, NORMAL_VARIABLE, start_position);
    private_name_scope->AddUnresolvedPrivateName(proxy);
    return impl()->factory()->NewVariableProxyExpression(proxy);
  }
  ...
  V8_INLINE VariableProxyExpression* ExpressionFromIdentifier(
    ...
    VariableProxy* proxy = impl()->expression_scope()->NewVariable(name, start_position);
    return impl()->factory()->NewVariableProxyExpression(proxy);
  }
  ...
  V8_INLINE void AddFormalParameter(ParserFormalParameters* parameters,
    ...
    auto parameter =
        parameters->scope->zone()->New<ParserFormalParameters::Parameter>(
            pattern, initializer, impl()->scanner()->location().beg_pos,
            initializer_end_position, is_rest);
  ...
  V8_INLINE void DeclareFormalParameters(ParserFormalParameters* parameters) {
    ...
    for (auto parameter : parameters->params) {
      ...
      // BuildParamerterInitializationBlock.
      scope->DeclareParameter(
          is_simple ? parameter->name() : impl()->ast_value_factory()->empty_string(),
          is_simple ? VariableMode::kVar : VariableMode::kTemporary,
          is_optional, parameter->is_rest(), impl()->ast_value_factory()->arguments_string(),
          parameter->position);
    }
  }
  ...
  V8_INLINE void ConvertBinaryToNaryOperationSourceRange(
    ...
    SourceRange range = ranges->GetRange(SourceRangeKind::kRight);
    source_range_map_->Insert(
        nary_op, impl()->zone()->template New<NaryOperationSourceRanges>(impl()->zone(), range));
  }
  ...
  V8_INLINE void RecordCaseClauseSourceRange(CaseClause* node,
    ...
    if (source_range_map_ == nullptr) return;
    source_range_map_->Insert(node,
                              impl()->zone()->template New<CaseClauseSourceRanges>(body_range));
  }
  ...
  V8_INLINE void RecordFunctionLiteralSourceRange(FunctionLiteral* node) {
    if (source_range_map_ == nullptr) return;
    source_range_map_->Insert(node, impl()->zone()->template New<FunctionLiteralSourceRanges>());
  }
  ...
  V8_INLINE void RecordSwitchStatementSourceRange(
    ...
    source_range_map_->Insert(
        ...
        impl()->zone()->template New<SwitchStatementSourceRanges>(continuation_position));
  }
  ...
  V8_INLINE void RecordThrowSourceRange(Statement* node,
    ...
    Throw* throw_expr = expr_stmt->expression()->AsThrow();
    source_range_map_->Insert(
        throw_expr, impl()->zone()->template New<ThrowSourceRanges>(continuation_position));
  }
...
#endif  // V8_PARSING_ABSTRACT_PARSER_H_
