...
class AbstractParser
 ...
 private:
  ...
  V8_INLINE Expression* ToString(Expression* expr) {
    ...
    args.Add(expr);
    return impl()->factory()->NewCallRuntime(Runtime::kInlineToStringRT, args,
                                             expr->position());
  }
  ...
  V8_INLINE ZonePtrList<Expression>* NewExpressionList(int size) const {
    return new (impl()->zone()) ZonePtrList<Expression>(size, impl()->zone());
  }
  V8_INLINE ZonePtrList<ObjectLiteral::Property>* NewObjectPropertyList(
      int size) const {
    return new (impl()->zone())
        ZonePtrList<ObjectLiteral::Property>(size, impl()->zone());
  }
  V8_INLINE ZonePtrList<ClassLiteral::Property>* NewClassPropertyList(
      int size) const {
    return new (impl()->zone())
        ZonePtrList<ClassLiteral::Property>(size, impl()->zone());
  }
  V8_INLINE ZonePtrList<Statement>* NewStatementList(int size) const {
    return new (impl()->zone()) ZonePtrList<Statement>(size, impl()->zone());
  }
  ...
  V8_INLINE void ConvertBinaryToNaryOperationSourceRange(
    ...
    source_range_map_->Insert(
        nary_op,
        new (impl()->zone()) NaryOperationSourceRanges(impl()->zone(), range));
  }
  ...
  V8_INLINE void RecordBlockSourceRange(Block* node,
    ...
    if (source_range_map_ == nullptr) return;
    source_range_map_->Insert(
        node, new (impl()->zone()) BlockSourceRanges(continuation_position));
  }
  ...
  V8_INLINE void RecordCaseClauseSourceRange(CaseClause* node,
    ...
    if (source_range_map_ == nullptr) return;
    source_range_map_->Insert(
        node, new (impl()->zone()) CaseClauseSourceRanges(body_range));
  }
  ...
  V8_INLINE void RecordConditionalSourceRange(Expression* node,
    ...
    source_range_map_->Insert(
        node->AsConditional(),
        new (impl()->zone()) ConditionalSourceRanges(then_range, else_range));
  }

  V8_INLINE void RecordFunctionLiteralSourceRange(FunctionLiteral* node) {
    if (source_range_map_ == nullptr) return;
    source_range_map_->Insert(node,
                              new (impl()->zone()) FunctionLiteralSourceRanges);
  }
  ...
  V8_INLINE void RecordJumpStatementSourceRange(Statement* node,
    ...
    source_range_map_->Insert(
        ...
        new (impl()->zone()) JumpStatementSourceRanges(continuation_position));
  }
  ...
  V8_INLINE void RecordIfStatementSourceRange(Statement* node,
    ...
    source_range_map_->Insert(
        node->AsIfStatement(),
        new (impl()->zone()) IfStatementSourceRanges(then_range, else_range));
  }
  ...
  V8_INLINE void RecordIterationStatementSourceRange(
    ...
    if (source_range_map_ == nullptr) return;
    source_range_map_->Insert(
        node, new (impl()->zone()) IterationStatementSourceRanges(body_range));
  }
  ...
  V8_INLINE void RecordSwitchStatementSourceRange(
    ...
    source_range_map_->Insert(node->AsSwitchStatement(),
                              new (impl()->zone()) SwitchStatementSourceRanges(
  ...
  V8_INLINE void RecordThrowSourceRange(Statement* node,
    ...
    source_range_map_->Insert(throw_expr,
                              new (impl()->zone())
                                  ThrowSourceRanges(continuation_position));
  }
  ...
  V8_INLINE void RecordTryCatchStatementSourceRange(
    ...
    if (source_range_map_ == nullptr) return;
    source_range_map_->Insert(
        node, new (impl()->zone()) TryCatchStatementSourceRanges(body_range));
  }
  ...
  V8_INLINE void RecordTryFinallyStatementSourceRange(
    ...
    if (source_range_map_ == nullptr) return;
    source_range_map_->Insert(
        node, new (impl()->zone()) TryFinallyStatementSourceRanges(body_range));
  }
...

  {
    ...
    if (call_super) {
      ...
      Expression* call;
      {
        ScopedPtrList<Expression> args(impl()->pointer_buffer());
        VariableProxy* proxy =
            impl()->factory()->NewVariableProxy(constructor_args);
        ...
        call = impl()->factory()->NewCall(super_call_ref, args, pos);
      }
...
Expression* AbstractParser<Impl>::NewSuperPropertyReference(int pos) {
  ...
  Expression* home_object_symbol_literal = impl()->factory()->NewSymbolLiteral(
      AstSymbol::kHomeObjectSymbol, kNoSourcePosition);
  Expression* home_object = impl()->factory()->NewProperty(
      this_function_proxy_expr, home_object_symbol_literal, pos);
  return impl()->factory()->NewSuperPropertyReference(home_object, pos);
}
...
void AbstractParser<Impl>::ParseProgram(
  ...
  RuntimeCallTimerScope runtime_timer(
      impl()->runtime_call_stats_, impl()->flags().is_eval()
...
void AbstractParser<Impl>::ParseFunction(
    Isolate* isolate, ParseInfo* info, Handle<SharedFunctionInfo> shared_info) {
  // It's OK to use the Isolate & counters here, since this function is only
  // called in the main thread.
  DCHECK(impl()->parsing_on_main_thread_);
  ...
  if (V8_UNLIKELY(shared_info->HasUncompiledDataWithBinAstParseData() ||
    ...
    FunctionLiteral* literal;
    {
      ...
      // printf("PREPARSE++: successfully deserialized %sfunction '", is_inner_binast ? "inner " : "");
  ...
  if (V8_UNLIKELY(result == nullptr && shared_info->private_name_lookup_skips_outer_class() &&
    ...
    if (deserialize_microseconds > 0) {
      ...
      printf("PREPARSE++: Parse time: %lld vs deserialize time: %lld (%lf%% change) for %d bytes %s\n", parse_microseconds, deserialize_microseconds, percent_change, function_length, is_inner_binast ? "(inner)" : "");
    }
  }
  MaybeResetCharacterStream(info, result);
  MaybeProcessSourceRanges(info, result, impl()->stack_limit_);
...
Statement* AbstractParser<Impl>::ParseModuleItem() {
  ...
  if (next == Token::IMPORT) {
    ...
    if ((!impl()->flags().allow_harmony_dynamic_import() ||
         peek_ahead != Token::LPAREN) &&
        (!impl()->flags().allow_harmony_import_meta() ||
         peek_ahead != Token::PERIOD)) {
...
AbstractParser<Impl>::ParseExportClause(Scanner::Location* reserved_loc) {
  // ExportClause :
  //   '{' '}'
  //   '{' ExportsList '}'
  //   '{' ExportsList ',' '}'
  ...
  ZoneChunkList<ExportClauseData>* export_data =
      new (impl()->zone()) ZoneChunkList<ExportClauseData>(impl()->zone());
  ...
  while ((name_tok = impl()->peek()) != Token::RBRACE) {
    ...
    const AstRawString* local_name = impl()->ParsePropertyName();
    ...
    if (impl()->CheckContextualKeyword(impl()->ast_value_factory()->as_string())) {
      export_name = impl()->ParsePropertyName();
...
AbstractParser<Impl>::ParseNamedImports(int pos) {
  ...
  auto result =
      new (impl()->zone()) ZonePtrList<const NamedImport>(1, impl()->zone());
  while (impl()->peek() != Token::RBRACE) {
    ...
    NamedImport* import =
        new (impl()->zone()) NamedImport(import_name, local_name, location);
...
void AbstractParser<Impl>::ParseImportDeclaration() {
  ...
  if (tok == Token::STRING) {
    ...
    impl()->module()->AddEmptyImport(module_specifier, specifier_loc);
    return;
  }
  ...
  if (import_default_binding != nullptr) {
    impl()->module()->AddImport(impl()->ast_value_factory()->default_string(),
                                ...
                                import_default_binding_loc, specifier_loc,
                                impl()->zone());
  }
    ...
    if (named_imports->length() == 0) {
      ...
      for (const NamedImport* import : *named_imports) {
        impl()->module()->AddImport(import->import_name, import->local_name,
                                    module_specifier, import->location,
...
void AbstractParser<Impl>::ParseExportStar() {
  ...
  if (!FLAG_harmony_namespace_exports ||
      !impl()->PeekContextualKeyword(impl()->ast_value_factory()->as_string())) {
    ...
    const AstRawString* module_specifier = ParseModuleSpecifier();
    impl()->ExpectSemicolon();
    impl()->module()->AddStarExport(module_specifier, loc, specifier_loc,
                                    impl()->zone());
    return;
  }
  ...
  impl()->ExpectContextualKeyword(impl()->ast_value_factory()->as_string());
  const AstRawString* export_name = impl()->ParsePropertyName();
  ...
  impl()->module()->AddStarImport(local_name, module_specifier, local_name_loc,
                                  specifier_loc, impl()->zone());
  impl()->module()->AddExport(local_name, export_name, export_name_loc,
                              impl()->zone());
}
  ...
  switch (impl()->peek()) {
    ...
    case Token::LBRACE: {
      ...
      if (impl()->CheckContextualKeyword(impl()->ast_value_factory()->from_string())) {
        specifier_loc = impl()->scanner()->peek_location();
      ...
      if (module_specifier == nullptr) {
        ...
        for (const ExportClauseData& data : *export_data) {
          impl()->module()->AddExport(data.local_name, data.export_name,
                                      module_specifier, data.location,
                                      specifier_loc, impl()->zone());
        }
      }
      return impl()->factory()->EmptyStatement();
...
FunctionLiteral* AbstractParser<Impl>::ParseFunctionLiteral(
  ...
  RuntimeCallTimerScope runtime_timer(
      impl()->runtime_call_stats_, RuntimeCallCounterId::kParseFunctionLiteral,
      RuntimeCallStats::kThreadSpecific);
  base::ElapsedTimer timer;
  if (V8_UNLIKELY(FLAG_log_function_events)) timer.Start();
...
void AbstractParser<Impl>::DeclarePublicClassField(
    ...
    bool is_computed_name, typename ParserBase<Impl>::ClassInfo* class_info) {
  if (is_static) {
    class_info->static_fields->Add(property, impl()->zone());
  } else {
...
template <typename Impl>
void AbstractParser<Impl>::DeclarePrivateClassMember(
    ClassScope* scope, const AstRawString* property_name,
    ClassLiteralProperty* property, ClassLiteralProperty::Kind kind,
    bool is_static, typename ParserBase<Impl>::ClassInfo* class_info) {
  DCHECK_IMPLIES(kind != ClassLiteralProperty::Kind::FIELD,
                 impl()->flags().allow_harmony_private_methods());
...
FunctionLiteral* AbstractParser<Impl>::CreateInitializerFunction(
  ...
  InitializeClassMembersStatement* stmt =
      impl()->factory()->NewInitializeClassMembersStatement(fields,
                                                            kNoSourcePosition);
  statements.Add(stmt);
  FunctionLiteral* result = impl()->factory()->NewFunctionLiteral(
      impl()->ast_value_factory()->GetOneByteString(name), scope, statements, 0,
...
AbstractParser<Impl>::OpenTemplateLiteral(int pos) {
  return new (impl()->zone()) TemplateLiteral(impl()->zone(), pos);
}
