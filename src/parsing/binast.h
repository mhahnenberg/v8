 ...
 public:
  BinAstNodeFactory(BinAstValueFactory* ast_value_factory, Zone* zone)
    : zone_(zone),
      ...
      empty_statement_(new (zone) class BinAstEmptyStatement()),
      this_expression_(new (zone) class BinAstThisExpression()),
      failure_expression_(new (zone) class BinAstFailureExpression())
  {
  }
  ...
  BinAstBlock* NewBlock(int capacity, bool ignore_completion_value) {
    return new (zone_) BinAstBlock(zone_, capacity, ignore_completion_value, false);
  }

  BinAstBlock* NewBlock(bool ignore_completion_value, bool is_breakable) {
    return new (zone_) BinAstBlock(ignore_completion_value, is_breakable);
  }
...
#define STATEMENT_WITH_POSITION(NodeType) \
  BinAst##NodeType* New##NodeType(int pos) { return new (zone_) BinAst##NodeType(pos); }
...
#undef STATEMENT_WITH_POSITION
  ...
  BinAstExpressionStatement* NewExpressionStatement(BinAstExpression* expression, int pos) {
    return new (zone_) BinAstExpressionStatement(expression, pos);
  }
  ...
  BinAstReturnStatement* NewReturnStatement(BinAstExpression* expression, int pos,
    ...
    return new (zone_) BinAstReturnStatement(expression, BinAstReturnStatement::kNormal,
                                       pos, end_position);
  }
  ...
  BinAstIfStatement* NewIfStatement(BinAstExpression* condition, BinAstStatement* then_statement,
                              BinAstStatement* else_statement, int pos) {
    return new (zone_)
        BinAstIfStatement(condition, then_statement, else_statement, pos);
  }
  ...
  BinAstLiteral* NewStringLiteral(const AstRawString* string, int pos) {
    ...
    return new (zone_) BinAstLiteral(string, pos);
  }

  // A JavaScript symbol (ECMA-262 edition 6).
  BinAstLiteral* NewSymbolLiteral(AstSymbol symbol, int pos) {
  ...
  BinAstLiteral* NewNumberLiteral(double number, int pos) {
    return new (zone_) BinAstLiteral(number, pos);
  }
  ...
  BinAstLiteral* NewBooleanLiteral(bool b, int pos) {
    return new (zone_) BinAstLiteral(b, pos);
  }

  BinAstLiteral* NewNullLiteral(int pos) {
    return new (zone_) BinAstLiteral(BinAstLiteral::kNull, pos);
  }

  BinAstLiteral* NewUndefinedLiteral(int pos) {
    return new (zone_) BinAstLiteral(BinAstLiteral::kUndefined, pos);
  }

  BinAstLiteral* NewTheHoleLiteral() {
    return new (zone_) BinAstLiteral(BinAstLiteral::kTheHole, kNoSourcePosition);
  }
  ...
  BinAstVariableProxyExpression* NewVariableProxyExpression(VariableProxy* proxy) {
    return new (zone_) BinAstVariableProxyExpression(proxy);
  }
  ...
  BinAstProperty* NewProperty(BinAstExpression* obj, BinAstExpression* key, int pos,
    ...
    return new (zone_) BinAstProperty(obj, key, pos, optional_chain);
  }


  BinAstCallNew* NewCallNew(BinAstExpression* expression,
                      const ScopedPtrList<BinAstExpression>& arguments, int pos) {
    return new (zone_) BinAstCallNew(zone_, expression, arguments, pos);
  }
  ...
  BinAstCall* NewCall(BinAstExpression* expression,
    ...
    return new (zone_)
        BinAstCall(zone_, expression, arguments, pos, converted_possibly_eval, optional_chain);
  }
  ...
  BinAstBinaryOperation* NewBinaryOperation(Token::Value op,
                                      ...
                                      int pos) {
    return new (zone_) BinAstBinaryOperation(op, left, right, pos);
  }
  ...
  BinAstCountOperation* NewCountOperation(Token::Value op,
                                    ...
                                    int pos) {
    return new (zone_) BinAstCountOperation(op, is_prefix, expr, pos);
  }
  ...
  BinAstCompareOperation* NewCompareOperation(Token::Value op,
                                        ...
                                        BinAstExpression* right,
                                        int pos) {
    return new (zone_) BinAstCompareOperation(op, left, right, pos);
  }
  ...
  BinAstAssignment* NewAssignment(Token::Value op,
    ...
    if (op == Token::ASSIGN || op == Token::INIT) {
      return new (zone_)
          BinAstAssignment(BinAstNode::kAssignment, op, target, value, pos);
    } else {
      return new (zone_) BinAstCompoundAssignment(
          op, target, value, pos,
  ...
  BinAstFunctionLiteral* NewFunctionLiteral(
    ...
    return new (zone_) BinAstFunctionLiteral(
